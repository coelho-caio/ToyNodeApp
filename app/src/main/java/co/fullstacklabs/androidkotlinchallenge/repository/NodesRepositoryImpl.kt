...
class NodesRepositoryImpl(
    ...
    @Throws(NetworkException::class)
    override suspend fun getNodeBlock(url: String): List<NodeBlockModel> {
        return when(val outcome = network.getNodeBlock(url = url).parseResponse()) {
            is Outcome.Success -> outcome.value.data.map { it.toDomain() }
            is Outcome.Failure -> throw NetworkException.parse(outcome.statusCode)
        }
    }
}